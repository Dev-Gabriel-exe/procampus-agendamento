...
import {
  ...
  ChevronDown, MapPin, CalendarDays, Clock, Copy,
} from 'lucide-react'
...
export default function RecuperacaoPage() {
  ...
  const [selSubjectsP,   setSelSubjectsP]   = useState<string[]>([]) // paralela: até 4
  ...
  // Paralela usa steps: 1→2(dados), Normal usa steps: 1→2(pix)→3(dados)
  ...
  async function loadSlots() {
    ...
    try {
      ...
      // Para paralela: busca todos os slots da série (disciplina é escolhida pelo pai)
        ...
        {step < successStep && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 20, textAlign: 'center' }}>
            <h1 style={{ fontFamily: '"Roboto Slab",serif', fontWeight: 900, fontSize: 24, color: 'white', margin: 0 }}>
              {step === 1 ? 'Recuperação' : step === dataStep ? 'Seus dados' : '💰 Pagamento'}
            </h1>
            <p style={{ color: 'rgba(255,255,255,0.35)', marginTop: 6, fontSize: 14, lineHeight: 1.4 }}>
              {step === 1 ? 'Escolha a série e o horário'
                : step === dataStep ? 'Preencha para confirmar a inscrição'
        ...
        <AnimatePresence mode="wait" custom={dir}>
          ...
          {step === 1 && (
            <motion.div key="s1" custom={dir} variants={slide} initial="enter" animate="center" exit="exit" transition={{ duration: 0.28, ease: [0.22,1,0.36,1] }} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {/* Disciplinas — só para paralela (Fund2) */}
              <AnimatePresence>
                  ...
                  {schedules.map((slot, idx) => {
                    ...
                    return (
                      <motion.button key={`${slot.id}-${idx}`} onClick={() => setSelectedSlot(slot)} whileTap={{ scale: 0.98 }}
                        ...
                        {isSel ? <Check style={{ width: 20, height: 20, color: '#4ade80', flexShrink: 0 }} /> : <ArrowRight style={{ width: 18, height: 18, color: 'rgba(255,255,255,0.18)', flexShrink: 0 }} />}
                      </motion.button>
                    )
                  })}
                </motion.div>
          ...
          {/* ══ STEP 2 — PIX (só normal/Fund1) ══ */}
          {step === 2 && !isParalela && (
            <motion.div key="s2pix" custom={dir} variants={slide} initial="enter" animate="center" exit="exit" transition={{ duration: 0.28, ease: [0.22,1,0.36,1] }} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {selectedSlot && (
                <div style={{ background: 'rgba(35,164,85,0.1)', border: '1px solid rgba(35,164,85,0.25)', borderRadius: 14, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div>
                    <p style={{ fontWeight: 700, fontSize: 14, color: 'white', margin: 0 }}>{selectedSlot.subjectName} — {selGrade}</p>
              ...
              <div style={{ ...card, borderColor: 'rgba(249,115,22,0.3)', background: 'rgba(249,115,22,0.06)' }}>
                ...
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', margin: '0 0 16px', lineHeight: 1.5 }}>Realize o pagamento via PIX e anexe o comprovante.</p>
                ...
                <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 8, textAlign: 'center' }}>Toque na chave para copiar</p>
              </div>
              ...
              <div style={card}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', background: pixFile ? 'rgba(34,197,94,0.1)' : 'rgba(255,255,255,0.04)', border: `2px dashed ${pixFile ? '#22c55e' : 'rgba(35,164,85,0.3)'}`, borderRadius: 14, padding: '16px', transition: 'all 0.2s', minHeight: 60 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 10, background: pixFile ? 'rgba(34,197,94,0.15)' : 'rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <BookMarked style={{ width: 18, height: 18, color: pixFile ? '#4ade80' : 'rgba(255,255,255,0.35)' }} />
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <p style={{ fontSize: 14, fontWeight: 600, color: pixFile ? '#86efac' : 'rgba(255,255,255,0.5)', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{pixFile ? pixFile.name : 'Anexar comprovante'}</p>
                    <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', margin: '2px 0 0' }}>PDF ou imagem da tela de pagamento</p>
                  </div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <motion.button disabled={!pixOk} onClick={() => goTo(3)} whileTap={{ scale: pixOk ? 0.97 : 1 }} style={{ ...btnPrimary(!pixOk), flex: 2 }}>
                  Continuar <ArrowRight style={{ width: 18, height: 18 }} />
                </motion.button>
              </div>
            </motion.div>
          ...
          {/* ══ STEP dados ══ */}
          {step === dataStep && (
            <motion.div key="sdata" custom={dir} variants={slide} initial="enter" animate="center" exit="exit" transition={{ duration: 0.28, ease: [0.22,1,0.36,1] }} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <motion.button onClick={() => goTo(step - 1)} whileTap={{ scale: 0.97 }} style={{ flex: 1, padding: '16px', borderRadius: 14, border: '1.5px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.6)', fontSize: 15, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, minHeight: 54 }}>
                  <ArrowLeft style={{ width: 16, height: 16 }} />Voltar
                </motion.button>
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                  ...
                  {[
                    ...
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, paddingBottom: i < arr.length - 1 ? 12 : 0, marginBottom: i < arr.length - 1 ? 12 : 0, borderBottom: i < arr.length - 1 ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
                      ...
                      <span style={{ fontSize: 13, fontWeight: (item as any).hl ? 700 : 500, textAlign: 'right', color: (item as any).hl ? '#4ade80' : 'rgba(255,255,255,0.75)', textTransform: (item as any).hl ? 'capitalize' as any : 'none' as any }}>{item.value}</span>
                    </div>
                  ))}
                </motion.div>
              )}
